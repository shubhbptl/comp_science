...
// Precondition: Creates an empty priority queue that can store integer data
// items
//    with different integer priority.  Higher
//    integer values indicate higher priority in the queue.  For example,
//    consider the priority and the data value to be key-value pairs where the
//    priority is the key and the data is the value.  The queue could hold 21,10
//    and 35, 5 so that the first item to be removed from the queue would be the
//    data value 5 because it has higher priority (35) than the data value 10
//    which only has (21).
...
// Postcondition: returns SUCCESS if the item was successfully added to the
// queue
...
// Precondition: hQueue is a handle to a valid priority queue opaque object.
// Postcondition: returns SUCCESS if the highest priority item was removed from
// the queue
...
// Precondition: hQueue is a handle to a valid priority queue opaque object.
// Postcondition: returns TRUE if the priority_queue is empty and FALSE
// otherwise.
Boolean priority_queue_is_empty(PRIORITY_QUEUE hQueue);

// Precondition: phQueue is a pointer to the handle of a valid priority queue
// opaque object. Postcondition: The opaque object will be free'd from memory
// and the handle pointed to
